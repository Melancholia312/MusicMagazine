...
class Album(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('album_detail', kwargs={'album_slug': self.slug, 'artist_slug': self.artist.slug})
...
class Order(models.Model):
    ...
    comment = models.TextField(verbose_name='Комменатрий к заказу', blank=True, null=True)
...
class Notification(models.Model):
    ...
    def __str__(self):
        return f'Уведолмение для {self.recipient.user.username} | id={self.id}'
